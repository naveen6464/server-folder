...
const verifyCallback = (req, resolve, reject, requiredRights) => async (
  ...
  if (requiredRights.length) {
    ...
    if (user.role.includes("admin")) {
      userRights = roleRights.get("admin");
    } else if (user.role.includes("approver")) {
      userRights = roleRights.get("approver");
    } else if (user.role.includes("sponsor")) {
      userRights = roleRights.get("sponsor");
    } else if (user.role.includes("resource")) {
      userRights = roleRights.get("resource");
    }
